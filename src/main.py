...
if __name__ == '__main__':
    #from env vars
    base = np.fromstring(os.getenv("BASE"), sep=',')
    microservices_mcl = np.fromstring(os.getenv("MICROSERVICES_MCL"), sep=',')
    microservices_mf = np.fromstring(os.getenv("MICROSERVICES_MF"), sep=',')
    delta_list = ast.literal_eval(os.getenv("DELTA_LIST"))
    parsed_rows = [list(map(int, item.split(','))) for item in delta_list]
    scale_config = np.array(parsed_rows)
    predictions_string = os.getenv("PREDICTIONS", "")
    predictions = []
    if predictions_string != "":
        predictions = list(map(int, predictions_string.split(',')))
    scores_string = os.getenv("SCORES", "")
    scores = []
    if scores_string != "":
